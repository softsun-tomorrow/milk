...
class GateWaySocketServer extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'gateway-worker:socket-server {action} {--daemon}';
     ...
     * @var string
     */
    protected $description = 'Start a GatewayWorker Socket Server.';
     ...
     *
     * php artisan_socket gateway-worker:socket-server start --daemon
     *
...
//            exit("start.php not support windows, please use start_for_win.bat\n");
        ...
        $argv[0] = 'gateway-worker:socket-server';
        $argv[1] = $action;
    ...
    private function startBusinessWorker()
    {
        ...
        $worker->name            = 'Socket BusinessWorker';                        #设置BusinessWorker进程的名称  worker名称,根据不同的项目自定义
    ...
    {
        $gateway = new Gateway(env('GATEWAY_SOCKET_NAME'));
        $gateway->name                 = 'Socket Gateway';                         #设置Gateway进程的名称，方便status命令中查看统计
        // gateway进程数（设置不合理会报错）
        //业务代码偏向IO密集型 进程数为cpu内核的3倍
        //业务代码偏向CPU密集型 进程数为cpu内核个数
        //不确定属于那种   内核的2倍
        $gateway->count                = 10;                                 #进程的数量
        $gateway->lanIp                = env('GATEWAY_SOCKET_LAN_IP');         #内网ip,多服务器分布式部署的时候需要填写真实的内网ip
        $gateway->startPort            = env('GATEWAY_SOCKET_START_PORT');     #监听本机端口的起始端口
        //服务端向客户端发送心跳数据的时间间隔 单位：秒。如果设置为0代表不发送心跳检测
        $gateway->pingInterval         = env('GATEWAY_SOCKET_HEART');
